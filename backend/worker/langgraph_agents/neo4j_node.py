    ...
    with neo4j_client.driver.session() as session:
        for skill_name in raw_skills:
            ...
            if result:
                ...
                logging.warning(f"  - Matched: '{skill_name}' -> '{canonical_name}' ({result['category']})")
            else:
                # Nếu không thấy trong Graph, giữ nguyên tên gốc (sẽ seed bổ sung sau)
                normalized_skills.append(skill_name)
                logging.warning(f"  - No match for: '{skill_name}', keeping original.")

    # Cập nhật kết quả bóc tách đã được chuẩn hóa
    state["parsed_data"]["skills"] = list(set(normalized_skills)) # Remove duplicates
