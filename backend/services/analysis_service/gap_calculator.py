...
class GapCalculator:
    ...
    async def calculate_gap_v2(self, user_id: str, cv_id: str, requirements_source: Any):
        ...
        results = {
            ...
            "recommendations": [],
            "notes": [f"Calculation Method: Advanced 3-Tier Algorithm ({len(valid_requirements)} items)"]
        }
        
        # Tạo recommendations từ breakdown
        for it in res["breakdown"]["partial"]:
            results["recommendations"].append({"skill": it["skill"], "type": "PARTIAL", "target_level": "Mid-level"})
        for it in res["breakdown"]["gap"]:
            results["recommendations"].append({"skill": it["skill"], "type": "MISSING", "target_level": "Mid-level"})
