...
def scan_conversation(conv_dir: Path, repo_ids: list[str]):
    """
...
def create_log_entries(conv_data: dict):
...
def main():
    parser = argparse.ArgumentParser(description="Antigravity IDE log scanner")
    parser.add_argument("--auto", action="store_true", help="Auto-scan recent sessions")
    parser.add_argument("--hours", type=int, default=24, help="Hours to look back")
    parser.add_argument("--conversation-id", help="Scan specific ID")
    parser.add_argument("--all", action="store_true", help="Scan everything")
    ...
    if not brain_dir:
        print("[antigravity-log] ⚠️ Brain directory not found.", file=sys.stderr)
        sys.exit(0)
    ...
    if args.conversation_id:
        ...
        if conv_dir.exists() and args.conversation_id not in logged_ids:
            conv_data = _extract_conversation_data(conv_dir, args.conversation_id)
            if conv_data: all_entries.extend(create_log_entries(conv_data))
    else:
        try:
        ...
        for conv_dir in conv_dirs:
            conv_id = conv_dir.name
            if conv_id in logged_ids: continue
            if cutoff and not args.all:
                try:
                    if datetime.fromtimestamp(conv_dir.stat().st_mtime, tz=VN_TZ) < cutoff: continue
    ...
    if not all_entries:
        print("[antigravity-log] ℹ️ No new Antigravity sessions found.", file=sys.stderr)
        sys.exit(0)
