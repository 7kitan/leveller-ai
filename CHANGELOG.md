...
### Added
- **Landing Page Overhaul**: Tái cấu trúc toàn diện trang chủ bám sát Pitching Deck ("Real signal on both sides").
    - Hỗ trợ Scroll Snap, tối ưu mobile responsiveness và chuẩn hóa Typography.
    - Tích hợp Screenshot Dashboard thực tế và biểu đồ Radar động.
- **Hạ tầng Production**:
    - Script cài đặt sản xuất (`setup-prod.sh`, `.bat`) và nạp dữ liệu mồi (3,000+ courses).
    - Triển khai **ConfigManager** động: Ưu tiên cấu hình Database > .env fallback.

### Changed
- **CV Service Stability**: Giải quyết triệt để lỗi **502 Bad Gateway** bằng cách tối ưu cơ chế polling và timeout.
- **Frontend Resilience**: Thêm modal cảnh báo quá tải (Overload modal) cho các trường hợp network timeout.

### Fixed
- **Database Schema**: Bổ sung `skill_id` thiếu trong bảng `youtube_video_skills` và ổn định Ingestion Pipeline.
- **Config**: Fix bug JSON parsing trong ConfigManager và đồng bộ hóa các tham số AI Inference Hub.
- **Documentation**: Hoàn thiện tái cấu trúc hệ thống Docs (**ADR-7**), tách biệt ARCHITECTURE.md và SETUP_GUIDE.md để tối ưu khả năng bảo trì.
...
- **Vector Preservation**: Triển khai hệ thống Export/Import JSON tích hợp Vector Embeddings (**ADR-3**), giúp migrate dữ liệu mà không tốn chi phí re-embedding.
- **Deduplication Strategy**: Triển khai chính sách **Skip duplicates** (**ADR-4**) cho các thao tác nạp dữ liệu bulk để bảo vệ tính toàn vẹn của database.
- **Bulk Operations UI**: Bổ sung hỗ trợ File Upload (**ADR-5**) và Custom Confirmation Modal (**ADR-6**) cho các tác vụ crawl dữ liệu quy mô lớn.
...
- **Logic**: BUG-007/008/012/013/015/022 (demand_filter, case-sensitivity, JD persistence, token logging).

## [1.4.0] - 2026-04-24

### Added
- **Thuật toán Tính Tăng Trưởng Dựa Trên Database**: Triển khai module `growth_calculator.py` mới để tính toán chính xác tiềm năng tăng trưởng (potential_match, market_demand_growth) dựa trên dữ liệu thực tế từ DB thay vì ước lượng từ LLM.
    - `calculate_skill_impact()`: Tính toán impact của từng skill dựa trên `JobSkillRequirement.importance_weight` và `MarketSkillStats.demand_score_premium_pct`.
    - `calculate_market_sentiment()`: Phân tích xu hướng thị trường dựa trên `growth_rate_30d` và `demand_score`.
    - Mỗi skill gap giờ hiển thị **match_impact** (+X% match) và **demand_impact** (+Y% demand) riêng biệt.
- **Hiển thị Impact Badge**: Thêm badges trực quan cho từng skill gap trên giao diện Recommend, giúp người dùng thấy rõ skill nào ảnh hưởng nhiều nhất đến tỷ lệ match và nhu cầu thị trường.
...
- **Cải tiến Thuật Toán Dự Báo**: Thay thế thuật toán heuristic cũ (chỉ đếm số lượng khóa học: `log2(course_count) * 6.5`) bằng thuật toán dựa trên trọng số thực tế từ Job Description và dữ liệu thị trường.
    ...
    - **Sau**: Python (+25% match, +12% demand) ≠ Excel (+3% match, +2% demand)
- **Tối ưu LLM Prompt**: Loại bỏ các trường `potential_match_pct` và `market_demand_pct` khỏi LLM prompt vì giờ được tính toán tự động bởi backend.
...
- **Lỗi TypeScript**: 
    ...
    - Thêm các fields mới vào `SkillGap` interface (`match_impact`, `market_demand`, `demand_score_range`).
- **Lỗi CSS Radar Chart**: Loại bỏ các CSS rules trùng lặp ghi đè chiều cao radar chart, đảm bảo sizing nhất quán.
- **Bug Market Stats Aggregator**: Fix logic extract skills từ `extracted_requirements_json` - trước đây chỉ lấy được `type="skill"` và bỏ qua toàn bộ skills trong `type="group"`, dẫn đến MarketSkillStats table trống. Sau khi fix, đã populate thành công 71 skills với đầy đủ demand premium, demand score, và growth rate.
...
- **Tool Stability**: Khắc phục lỗi **Infinite Loop** khi gọi Tool AI bằng cách thêm stop condition (max_iterations) và cơ chế xử lý lỗi network.
